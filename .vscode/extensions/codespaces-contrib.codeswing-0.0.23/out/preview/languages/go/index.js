...
async function compileGo(code, documentUri) {
    try {
        ...
        const response = await axios_1.default.post(COMPILE_URL, data, {
        ...
        if (Errors) {
            diagnosticCollection = vscode.languages.createDiagnosticCollection("CodeSwing");
            let match, diagnostics = [];
            while ((match = ERROR_PATTERN.exec(Errors)) !== null) {
                ...
                new vscode.Diagnostic(new vscode.Range(lineNumber, columnNumber, lineNumber, columnNumber), message, vscode.DiagnosticSeverity.Error);
            }
