...
function isSwingDocument(uri) {
    const swingUri = store_1.store.activeSwing.currentUri;
    if (!localeCompare(swingUri.scheme, uri.scheme) ||
...
function isSwingDocumentOfType(document, fileType) {
    ...
    switch (fileType) {
        case store_1.SwingFileType.markup:
            fileCandidates = markup_1.getCandidateMarkupFilenames();
            break;
    ...
    if (!fileCandidates && extensions) {
        fileCandidates = extensions.map((extension) => `${fileBaseName}${extension}`);
    }
...
async function openSwing(uri) {
    ...
    const isWorkspaceSwing = vscode.workspace.workspaceFolders &&
    ...
    if (manifest.tutorial) {
        ...
        const tutoralSteps = files.filter((file) => file.match(TUTORIAL_STEP_PATTERN));
    ...
    const layoutManager = await layoutManager_1.createLayoutManager(includedFiles, manifest.layout);
    ...
    const webViewPanel = vscode.window.createWebviewPanel(`${constants_1.EXTENSION_NAME}.preview`, "CodeSwing", { viewColumn: layoutManager.previewViewColumn, preserveFocus: true }, {
        ...
        localResourceRoots: [
            uri,
            currentUri,
            vscode_1.Uri.parse(`${proxyFileSystemProvider_1.ProxyFileSystemProvider.SCHEME}:/`),
        ],
    });
    ...
    const documentChangeDisposable = vscode.workspace.onDidChangeTextDocument(debounce_1.debounce(async ({ document }) => {
        if (isSwingDocumentOfType(document, store_1.SwingFileType.markup)) {
            const content = await markup_1.getMarkupContent(document);
            if (content !== null) {
                htmlView.updateHTML(content, runOnEdit);
            }
        }
        else if (isSwingDocumentOfType(document, store_1.SwingFileType.script)) {
        ...
        else if (isSwingDocumentOfType(document, store_1.SwingFileType.manifest)) {
        ...
        else if (isSwingDocumentOfType(document, store_1.SwingFileType.stylesheet)) {
            const content = await stylesheet_1.getStylesheetContent(document);
            if (content !== null) {
                htmlView.updateCSS(content, runOnEdit);
            }
        }
        else if (isSwingDocumentOfType(document, store_1.SwingFileType.readme)) {
            const rawContent = document.getText();
            processReadme(rawContent, runOnEdit);
        }
        else if (isSwingDocumentOfType(document, store_1.SwingFileType.config)) {
    ...
    if (!runOnEdit && autoRun === "onSave") {
        documentSaveDisposeable = vscode.workspace.onDidSaveTextDocument((document) => {
            if (isSwingDocument(document.uri)) {
                htmlView.rebuildWebview();
            }
        });
...
function registerPreviewModule(context, api, syncKeys) {
    ...
    tutorials_1.registerTutorialModule(context, syncKeys);
}
